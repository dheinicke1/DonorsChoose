...
def tokenizer(text):
    ...
    tokens = [w for w in tokens if not w in [u"the", u"The",u"'s", u"n't", u"...", u"''", u"``", u"'",u"\u2014", u"\u2026", u"\u2013"]]

    # Remove NaN from essays
    tokens = [w for w in tokens if not w in [u"nan"]]
...
## Pre-process text
# 1) Extract featuretures from the text: Charagter legth and word count of each essy
...
#df_train['essay_token'] = df_train.apply(lambda row : tokenizer(row['essay']), axis=1)
...
df_train = pd.read_csv('train_preprocessed.csv', dtype=dtypes_preprocessed, encoding='ISO-8859-1')
df_test =  pd.read_csv('test_preprocessed.csv', dtype=dtypes_preprocessed, encoding='ISO-8859-1')
...
# tfidf_vectorizer = TfidfVectorizer(tokenizer=tokenizer, min_df=10, ngram_range=(1, 2))
...
'''
# Westons dumb aint thing
counter = 0
aint_essay = 0
aint_gonna_get_it = 0

for esssay_num in ['project_essay_1','project_essay_2','project_essay_3','project_essay_4']:
    ind = 0
    for essay in df_train[esssay_num].values:
        if type(essay) == str:
            counter += essay.count(" ain't ")
            if essay.count(" ain't ") > 0:
                aint_essay +=1
                aint_gonna_get_it += df_train.project_is_approved[ind]
        ind += 1
    
print('Rate of success for essays with aint: %f' %(aint_gonna_get_it / aint_essay))
print('Rate of success of all essays: %f' %(sum(df_train.project_is_approved) / len(df_train)))

# Essays: Did the acceptance rate change when the essay requirements changed?
# The acceptance rate dropped by about 2% after only two essays were required. 
# Its unclear yet whether this is significant.

df_four_essay = df_train[df_train.project_essay_3.notnull()]
df_two_essay = df_train[df_train.project_essay_3.isnull()]
assert len(df_train) == len(df_four_essay) + len(df_two_essay)

print('Acceptance Rate with 4 Essays: %f' %(sum(df_four_essay.project_is_approved) / len(df_four_essay)))
print('Acceptance Rate with 2 Essays: %f' %(sum(df_two_essay.project_is_approved) / len(df_two_essay)))
